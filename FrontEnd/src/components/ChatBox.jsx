...
const ChatBox = ({ user, classID, onClose }) => {
  ...
  const fetchMemberMap = async () => {
    ...
    if (!error && data.length) {
      ...
      data.forEach((member) => {
        map[member.id] =
          member.role === "Tutor" ? "Tutor of class" : member.user_name;
      });
  ...
  return (
    <div className="fixed bottom-4 right-2 w-full max-w-xs ml-1 md:max-w-xs lg:max-w-sm bg-white shadow-lg border border-gray-300 rounded-lg z-50 flex flex-col h-[520px]">
      {/* Header */}
        ...
        {messages.map((msg) => {
          ...
          return (
            <div
              key={msg.id}
              className={`max-w-[80%] ${
                isMe ? "ml-auto text-right" : "text-left"
              }`}
            >
      ...
      {/* Input */}
      <div className="p-2 border-t flex flex-row">
        <input
          value={newMessage}
          ...
          className="text-black px-3 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400"
        />
        <div className="ml-[4px] w-[48px]">
          <button
            onClick={sendMessage}
